# Change Device Settings from the REST API
...
This guide will walk you through changing your device's settings directly through the AutoPi REST API. You'll learn how to retrieve available configuration options, view current settings states, and modify settings for your device without using the AutoPi Cloud interface. The guide covers authentication, endpoint usage, and synchronizing changes to your device.

**Prerequisites:**
- Authentication token (JWT or API Token) — see [Authentication](/getting_started/api/index.md).
- Device ID (primary key UUID, not unit ID) — retrieve via `GET /dongle/devices`.
- API URL: `https://api.autopi.io/` (or your dedicated environment URL for business customers).
