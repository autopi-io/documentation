...
The production build automatically emits machine-readable copies of the docs for
customers and AI tools to ingest into their own RAG systems:

- <https://docs.autopi.io/llms.txt> &mdash; index of the documentation, following
  the [llmstxt.org](https://llmstxt.org/) standard, with links to the Markdown
  version of each page.
- <https://docs.autopi.io/llms-full.txt> &mdash; the entire AutoPi documentation
  concatenated into a single Markdown document, suitable for direct LLM/RAG
  ingestion.
- Every documentation page is also served as clean Markdown alongside its HTML
  counterpart (e.g. `https://docs.autopi.io/cloud/index.md` or `https://docs.autopi.io/cloud/accounts.md`).

These files are generated by
[`docusaurus-plugin-llms`](https://www.npmjs.com/package/docusaurus-plugin-llms)
during `npm run build`, so they stay in sync with the site content and require
no separate step. Customers can point their AI tooling directly at the URLs
above to keep their AutoPi knowledge base up to date without us having to host a
vector database or RAG backend.
