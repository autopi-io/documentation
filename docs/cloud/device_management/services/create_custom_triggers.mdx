...
1. Let's start by creating a custom code module with the name *my_triggers* of type *Execution*. We
  can do that by going over to **Select device** > **Custom Code**:
...
2. Next, we need to register that trigger in the service that it'll be used. For this guide, we
  need to register it in the OBD service named **obd_manager**. Go to **Select device** > **Services** >
...
4. We now need to use the newly registered trigger hook by adding it to a workflow in a worker.
  Go to **Select device** > **Services** > **obd_manager** > **Workers** > **Create**:
  ...
  [this](/getting_started/developer_guides/autopi_logs.md) guide out for more information on how to retrieve the logs from your
  device.
  :::
...
to the cloud and be visible from **Select device** > **Events**.
