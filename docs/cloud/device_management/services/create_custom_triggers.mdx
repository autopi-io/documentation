...
1. Let's start by creating a custom code module with the name *my_triggers* of type *Execution*. We
  ...
  ![Editing custom code](/img/guides/create_custom_triggers/editing_custom_code.png) 
...
2. Next, we need to register that trigger in the service that it'll be used. For this guide, we
  ...
  **obd_manager** > **Hooks** > **Create**:
  ...
  ![Creating a custom hook](/img/guides/create_custom_triggers/creating_a_custom_hook.png) 
...
4. We now need to use the newly registered trigger hook by adding it to a workflow in a worker.
  ...
  ![Creating a new worker](/img/guides/create_custom_triggers/creating_a_new_worker.png) 
  ...
  [this](../../../guides/autopi_logs.md) guide out for more information on how to retrieve the logs from your
  device.
  :::
