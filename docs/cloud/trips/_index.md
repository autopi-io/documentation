...
Trips are automatically detected based on the events sent from the device, by default the [engine events](/events/vehicle/#engine-events) are used. 

It is fully customizeable what events trigger a new trip to be created and it can be changed in the [advanced settings](/configuration/trip/#event).
