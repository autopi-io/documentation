...
Finally the Raspberry Pi communicates directly with the SPM and performs power state changes based on more [high-level](/power/high_level) decisions.  
...
See the STN command for available power trigger options [here](/commands/stn).
