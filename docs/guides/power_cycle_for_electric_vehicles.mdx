...
title: Power Cycle for Electric Vehicles
---
...
Behind the scene this works by using [engine events](/cloud/events/vehicle.md/#engine-events).
...
support readout of engine RPM in exactly the same way according to the OBD-II PID standard
(SAE J1979).
...
1. if your vehicle is able to respond to PID requests and/or 
2. if your car continuously delivers data on a CAN bus.
...
Go to **Car Explorer** > **Loggers** > **Create** > **PID**. Choose your RPM PID and be sure to
select the *rpm_motor_event* trigger.

![Setting up RPM based PID logger](/img/guides/power_cycle_for_electric_vehicles/setting_up_rpm_based_pid_logger.png) 

After the logger has been synced to your device, and the service has been restarted, you should
start seeing motor events in the **Advanced** > **Events** page.

[Click here for documentation on motor events](/cloud/events/vehicle.md/#motor-events).
...
Go to **Car Explorer** > **Loggers** > **Create** > **PID**. Choose your arbitrary PID and be sure
to select the `communication_event` trigger.

![Setting up communication based PID logger](/img/guides/power_cycle_for_electric_vehicles/setting_up_communication_based_pid_logger.png) 

[Click here for documentation on communication events](/cloud/events/vehicle.md/#communication-events).

Next step is to re-configure the sleep timer functionality to use communication events instead of
the default engine/motor events. Go to **Settings** > **Advanced** > **Power** and change the three
values:
...
![Change events 1](/img/guides/power_cycle_for_electric_vehicles/change_events_1.png) 

...

![Change events 2](/img/guides/power_cycle_for_electric_vehicles/change_events_2.png) 
...
[here](/guides/services/create_custom_service_triggers.mdx).
...
[click here for documentation on engine events](/cloud/events/vehicle.md/#engine-events).

**Precondition:** A CAN message mapping for your specific vehicle which contains an RPM signal. If
you do not know any CAN messages for your vehicle, you can always check the *Community Library*
on https://my.autopi.io.

The following example shows how an RPM signal can be found in the *Community Library* for a Renault
Zoe. Go to **Car Explorer** > **Library** > **Community Library** and specify *Make* and *Model*:

![Community library screenshot](/img/guides/power_cycle_for_electric_vehicles/community_library_screenshot.png) 

Click on the *RPM* signal and in the popup window click on the button *Add to my library*. You have
now added the CAN message to your library and you can use it for logging.

Next step is to setup a CAN logger for the RPM signal. Go to **Car Explorer** > **Loggers** >
**Create** > **CAN** and choose the RPM signal:

![Setting up CAN signal logger](/img/guides/power_cycle_for_electric_vehicles/setting_up_can_signal_logger.png) 
...
After the logger has been synced to your device, and the service has been restarted, you should
start seeing engine events in the **Advanced** > **Events** page.

<Discussion
  title="How to setup power cycle for an electric vehicle"
  url="https://community.autopi.io/t/guide-how-to-setup-power-cycle-for-an-electric-vehicle/1668"
/>